...
public class Problem13 {
    ...
    /**
     * Method to convert roman literal to number
     *
     * @param s
     */
    private static int romanToInt(String s) {
        ...
        for (i = 0; i < s.length() - 1; i++) {
            ...
            if (romanNumeralMap.get(next) > romanNumeralMap.get(current)) {
                number += romanNumeralMap.get(next) - romanNumeralMap.get(current);
                i++;
            } else {
                number += romanNumeralMap.get(current);
            }
        }
        if (i < s.length()) {
            number += romanNumeralMap.get(s.charAt(s.length() - 1));
        }
        return number;
    }
